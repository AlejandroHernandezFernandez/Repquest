"use strict";
// Repquest's editable app source. Search for the section headings below to find
...
const ic = (n, size = 22) =>
  `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${icons[n] || icons.lift}</svg>`;
const esc = (s) =>
  String(s).replace(
    /[&<>"']/g,
    (c) =>
            ...
            r.reps >=
              Math.max(
                ...p.sets
                  .filter(
                    (x) =>
                      x.weight === Math.max(...p.sets.map((x) => x.weight)),
                  )
                  .map((x) => x.reps),
              ),
        );
        const moreReps = e.sets.some(
          (r) =>
            ...
            r.reps >
              Math.max(
                ...p.sets
                  .filter((x) => x.weight === r.weight)
                  .map((x) => x.reps),
              ),
        );
        const repVolume = [...new Set(e.sets.map((x) => x.weight))].some(
          (w) => {
            ...
            return (
              a.length === b.length &&
              b.length > 0 &&
              a.reduce((n, x) => n + x.reps, 0) >
                b.reduce((n, x) => n + x.reps, 0)
            );
          },
        );
        if (heavier) {
          ...
          p.sets.every(
            (x, i) => e.sets[i].weight >= x.weight && e.sets[i].reps >= x.reps,
          )
        ) {
          bonus = 5;
...
function durationLabel(session) {
  return Number.isSafeInteger(session.durationSeconds)
    ? `<span class="duration-label">Duration · ${formatDuration(session.durationSeconds)}</span>`
    : "";
}
...
function render() {
  ...
  $("#app").innerHTML =
    `<div class="shell"><aside class="sidebar"><div class="brand">${ic("lift", 32)}repquest</div><nav class="nav" aria-label="Main navigation">${[
      ["home", "Overview", "home"],
      ...
      .map(
        ([v, n, i]) =>
          `<button data-view="${v}" class="${view === v ? "active" : ""}" ${view === v ? 'aria-current="page"' : ""}>${ic(i)}${n}</button>`,
      )
      .join(
        "",
      )}</nav><div class="sidefoot"><span class="avatar">A</span><strong>Your training space</strong><p>Small wins. Stronger you.</p></div></aside><main class="main"><header class="topbar"><span class="eyebrow">YOUR PERSONAL TRAINING LOG</span><div class="brand mobilebrand">${ic("lift")}repquest</div><div class="topstats"><span class="gold">${ic("bolt")} ${r.total} XP</span><span class="green">${ic("trophy")} Level ${level}</span><span class="muted">${ic("target")} ${data.sessions.length} workouts</span></div></header>${loadError ? `<p class="notice error">${esc(loadError)}</p>` : ""}${view === "home" ? home(r) : view === "workout" ? workout() : view === "progress" ? progress() : view === "history" ? history(r) : settings()}</main></div>`;
  document
...
function home(r) {
  ...
  return `<div class="heading"><div><div class="eyebrow">LET'S BUILD SOME MOMENTUM</div><h1>Every rep counts.</h1><p class="sub">Show up. Get stronger. Earn your next level.</p></div><span class="pill">${new Date().toLocaleDateString(undefined, { weekday: "short", month: "short", day: "numeric" })}</span></div><div class="layout"><div class="stack"><section class="card"><div class="row"><h2>This week's quest</h2><span class="xpchip">${count} / ${data.goal} workouts</span></div><div class="week">${days.map((d, i) => `<div class="day ${d === today() ? "today" : ""} ${done.has(d) ? "done" : ""}"><span>${["MON", "TUE", "WED", "THU", "FRI", "SAT", "SUN"][i]}</span><div class="daycircle">${done.has(d) ? ic("check", 20) : new Date(d + "T12:00:00").getDate()}</div></div>`).join("")}</div><div class="row small"><span class="muted">${count >= data.goal ? "Weekly goal complete. You earned that rest." : `${data.goal - count} more workout${data.goal - count === 1 ? "" : "s"} to reach your goal`}</span><span class="green">${Math.min(100, Math.round((count / data.goal) * 100))}%</span></div><div class="progressbar"><span style="width:${Math.min(100, (count / data.goal) * 100)}%"></span></div></section><section class="card startcard"><span class="eyebrow" style="color:#75a447">YOUR NEXT WIN STARTS HERE</span><h2>${data.draft ? "Your workout is waiting." : "Ready for a little stronger?"}</h2><p>${data.draft ? "Pick up where you left off. Your sets are saved as you go." : "A good session starts with showing up. Take the first set from here."}</p><button class="primary" id="start">${ic("plus", 20)} ${data.draft ? "Continue workout" : "Start a workout"}</button></section><div class="statgrid">${[
    ...
    .map(
      ([i, n, l]) =>
        `<div class="card stat"><div class="iconbox">${ic(i)}</div><div class="number">${n}</div><p>${l}</p></div>`,
    )
    .join(
      "",
    )}</div><section class="card"><div class="sectionlabel"><h2>Recent activity</h2><button class="textbtn" data-view="history">View all</button></div>${data.sessions.length ? recent(r, 3) : `<div class="empty"><div class="iconbox">${ic("history")}</div><strong>A fresh start looks good on you.</strong>Your completed workouts will live here.</div>`}</section></div><aside class="stack aside"><section class="card level"><div class="eyebrow">YOUR STRENGTH JOURNEY</div><div class="levelbadge"><span>${Math.floor(r.total / 150) + 1}</span></div><h3>${["Getting started", "Building momentum", "Finding your rhythm", "Stronger every week"][Math.min(3, Math.floor(r.total / 450))]}</h3><p class="muted small">Level ${Math.floor(r.total / 150) + 1} · ${r.total} total XP</p><div class="progressbar"><span style="width:${((r.total % 150) / 150) * 100}%"></span></div><div class="row small muted"><span>${r.total % 150} / 150 XP</span><span>Level ${Math.floor(r.total / 150) + 2}</span></div></section><section class="card"><h3>Good effort. Real rewards.</h3><div class="rule">${ic("check")} Show up & finish <strong>+25 XP</strong></div><div class="rule">${ic("chart")} More reps, same weight <strong>+10 XP</strong></div><div class="rule">${ic("lift")} More weight, same reps <strong>+15 XP</strong></div><div class="rule">${ic("plus")} Extra working set <strong>+5 XP</strong></div><p class="small muted" style="margin-bottom:0">Consistency counts. Rest days don't take away your XP.</p></section><div class="notice tip">Your first session sets the baseline. Come back to see exactly how you've improved.</div></aside></div>`;
}
function recent(r, limit) {
  return [...sorted()]
    ...
    .map(
      (s) =>
        `<div class="historyrow"><div><strong>${esc(s.name)}</strong><p class="small muted">${fmt(s.date)} · ${s.exercises.length} exercises · ${s.exercises.reduce((n, e) => n + e.sets.length, 0)} sets${Number.isSafeInteger(s.durationSeconds) ? " · " + formatDuration(s.durationSeconds) : ""}</p></div><span class="xpchip">+${r.byId[s.id].xp} XP</span></div>`,
    )
    .join("");
}
...
function startModal() {
  show(
    `<h2>What's the plan?</h2><p>Choose a starting point. You can add or remove any exercise.</p><div class="choices">${Object.entries(
      routines,
    )
      .map(
        ([n, ex]) =>
          `<button class="secondary" data-routine="${n}">${n}<small>${ex.join(" · ")}</small></button>`,
      )
      .join(
        "",
      )}<button class="secondary" data-routine="Custom">Empty workout<small>Build your own session</small></button></div>`,
  );
  document
...
function workout() {
  if (!data.draft)
    return `<div class="heading"><div><div class="eyebrow">ONE SET AT A TIME</div><h1>Your workout</h1><p class="sub">Your last numbers, right beside your next ones.</p></div></div><section class="card startcard"><h2>Let's get to work.</h2><p>Choose a session, log your sets, and build on your last workout.</p><button class="primary" id="start">${ic("plus")} Start a workout</button></section>`;
  const d = data.draft;
  return `<div class="heading"><div><div class="eyebrow">WORKOUT IN PROGRESS</div><h1>${esc(d.name)}</h1><p class="sub">Check off each working set when you're done.</p></div><input class="datecontrol" aria-label="Workout date" type="date" id="sessiondate" value="${d.date}" max="${today()}"></div>${workoutClock(d)}<div class="layout"><div><div class="field"><label for="workname">Workout name</label><input id="workname" maxlength="100" value="${esc(d.name)}"></div>${d.exercises
    .map((e, i) => {
      const p = previous(e.name, d.date);
      return `<section class="card exercise"><div class="row exercisehead"><div><h3>${esc(e.name)}</h3><div class="previous">${p ? "Last: " + p.sets.map((s) => `${s.weight} ${data.unit} × ${s.reps}`).join(" · ") : "First session · set your baseline"}</div></div><button class="remove" data-remove="${i}" aria-label="Remove ${esc(e.name)}">×</button></div><div class="setgrid"><span class="label">SET</span><span class="label">${data.unit.toUpperCase()}</span><span class="label">REPS</span><span class="label">DONE</span></div>${e.sets.map((s, j) => `<div class="setgrid"><span class="setnum">${j + 1}</span><input type="number" inputmode="decimal" min="0" max="2000" step="0.5" placeholder="0" aria-label="${esc(e.name)} set ${j + 1} weight" data-ex="${i}" data-set="${j}" data-field="weight" value="${s.weight}"><input type="number" inputmode="numeric" min="1" max="500" step="1" placeholder="0" aria-label="${esc(e.name)} set ${j + 1} reps" data-ex="${i}" data-set="${j}" data-field="reps" value="${s.reps}"><input type="checkbox" aria-label="Complete ${esc(e.name)} set ${j + 1}" data-ex="${i}" data-set="${j}" data-field="done" ${s.done ? "checked" : ""}></div>`).join("")}<div class="row"><button class="textbtn" data-addset="${i}">+ Add set</button>${e.sets.length > 1 ? `<button class="textbtn muted" data-removeset="${i}">Remove last set</button>` : ""}</div></section>`;
    })
    .join(
      "",
...
function chart(points, unit) {
  if (points.length < 2)
    return `<div class="empty">${points.length ? "One point on the board. Log another day to see your trend." : "Your progress chart starts with your first entry."}</div>`;
  ...
  return `<svg class="chart" viewBox="0 0 ${W} ${H}" role="img" aria-label="${esc(unit)} trend from ${vals[0]} to ${vals.at(-1)}"><line x1="52" y1="30" x2="562" y2="30" stroke="#e9eee4" stroke-dasharray="4 4"/><line x1="52" y1="125" x2="562" y2="125" stroke="#e9eee4"/><text x="0" y="35">${max.toFixed(1)}</text><text x="0" y="130">${min.toFixed(1)}</text><polyline points="${coords.map((p) => p.join(",")).join(" ")}" fill="none" stroke="#58bf13" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>${coords.map((p, i) => `<circle cx="${p[0]}" cy="${p[1]}" r="4" fill="#58bf13"><title>${points[i].date}: ${points[i].v} ${unit}</title></circle>`).join("")}<text x="52" y="161">${fmt(points[0].date)}</text><text x="562" y="161" text-anchor="end">${fmt(points.at(-1).date)}</text></svg>`;
}
function progress() {
  const names = [
  ...
  return `<div class="heading"><div><div class="eyebrow">THE BIGGER PICTURE</div><h1>Stronger over time.</h1><p class="sub">Small changes add up. Here's your proof.</p></div></div><div class="stack"><section class="card"><div class="row"><h2>Lift progress</h2>${names.length ? `<select id="exercisechart" aria-label="Choose exercise">${names.map((n) => `<option ${selected === n ? "selected" : ""}>${esc(n)}</option>`).join("")}</select>` : ""}</div><p class="small muted">Heaviest working set per session · ${data.unit}. Check your reps in History for the full picture.</p>${chart(pts, data.unit)}</section><section class="card"><div class="row"><h2>Bodyweight</h2><span class="weightnumber">${data.weights.length ? data.weights.at(-1).value + ' <span class="small muted">' + data.unit + "</span>" : "—"}</span></div>${chart(
    data.weights.map((w) => ({ date: w.date, v: w.value })),
    data.unit,
  )}<form id="weightform" class="inlineform"><div class="field"><label for="weightdate">Date</label><input type="date" id="weightdate" max="${today()}" value="${today()}" required></div><div class="field"><label for="bodyweight">Weight (${data.unit})</label><input type="number" inputmode="decimal" id="bodyweight" min="1" max="1500" step="0.1" placeholder="e.g. ${data.unit === "lb" ? "170" : "77"}" required></div><button class="primary">Log weight</button></form><p class="small muted">One entry per day; logging again updates it. Bodyweight changes don't award XP.</p>${
    data.weights.length
      ? `<details><summary class="small muted">View ${data.weights.length} entries</summary>${[
          ...data.weights,
        ]
          .reverse()
          .map(
            (w) =>
              `<div class="historyrow"><span>${fmt(w.date)} · ${w.value} ${data.unit}</span><button class="textbtn" data-delweight="${w.date}">Delete</button></div>`,
          )
          .join("")}</details>`
      : ""
  }</section></div>`;
...
function history(r) {
  return `<div class="heading"><div><div class="eyebrow">THE WORK YOU PUT IN</div><h1>Workout history</h1><p class="sub">${data.sessions.length} sessions. Every one a step forward.</p></div><button class="secondary" id="start">+ Workout</button></div><section class="card">${
    data.sessions.length
      ? [...sorted()]
          .reverse()
          .map(
            (s) =>
              `<details class="historydetail"><summary><strong>${esc(s.name)}</strong> <span class="small muted">· ${fmt(s.date)}</span>${durationLabel(s)} <span class="xpchip">+${r.byId[s.id].xp} XP</span></summary><ul>${s.exercises.map((e) => `<li><strong>${esc(e.name)}</strong><br>${e.sets.map((x) => `${x.weight} ${data.unit} × ${x.reps}`).join(" · ")}</li>`).join("")}</ul><div class="notice">${r.byId[s.id].reasons.map(esc).join("<br>") || "Already earned today’s completion XP. Your work is still logged."}</div><button class="textbtn error" data-delete="${s.id}">Delete workout</button></details>`,
          )
          .join("")
...
function confirmAction(title, description, fn) {
  show(
    `<h2>${title}</h2><p>${description}</p><div class="workactions"><button class="secondary" id="cancel">Cancel</button><button class="primary" id="confirm">Confirm</button></div>`,
  );
...
function finish() {
  ...
  if (!exercises.length)
    return toast("Check off at least one completed set first.");
  if (
    exercises.some((e) =>
      e.sets.some(
        (s) =>
    ...
    return toast(
      "Completed sets need a weight of 0–2000 and 1–500 whole reps.",
    );
  const s = {
    ...
    exercises,
    ...(hasStartTime(d)
      ? { durationSeconds: getElapsedSeconds(d.startedAt) }
      : {}),
  };
  ...
  go("home");
  show(
    `<div class="iconbox" style="margin-bottom:20px">${ic("trophy")}</div><h2>That's another win.</h2><p>You showed up and put in the work.</p>${durationLabel(s)}<div class="levelbadge" style="font-size:1.7rem;">+${r.xp}</div><p style="text-align:center">XP earned</p><div class="notice">${r.reasons.map(esc).join("<br>") || "Another session logged. Today’s completion XP was already earned."}</div><button class="primary full" style="margin-top:20px" id="celebrate">Keep the momentum</button>`,
  );
...
function bind() {
  ...
  if ($("#addexercise"))
    $("#addexercise").onclick = () => {
      ...
      show(
        `<h2>Add an exercise</h2><form id="exerciseform"><div class="field"><label for="exname">Exercise name</label><input id="exname" list="exnames" maxlength="80" placeholder="e.g. Bench press" required autocomplete="off"><datalist id="exnames">${names.map((n) => `<option value="${esc(n)}">`).join("")}</datalist></div><button class="primary full">Add exercise</button></form>`,
      );
  ...
  document.querySelectorAll("[data-field]").forEach(
    (el) =>
      (el.onchange = () => {
        data.draft.exercises[el.dataset.ex].sets[el.dataset.set][
          el.dataset.field
