...
elements.set("#celebrate", node());
elements.set("#toast", node());
...
now += 1500000;
elements.set("#workout-timer", node());
run("syncTimer()");
assert.equal(elements.get("#workout-timer").textContent, "0:25:00");
run("syncTimer();syncTimer()");
assert.equal(intervals.size, 1);
elements.delete("#workout-timer");
run("syncTimer()");
...
failSave = true;
run("finish()");
assert.equal(run("data.sessions.length"), 0);
assert.ok(run("data.draft"));
failSave = false;
run("finish()");
assert.equal(run("data.sessions[0].durationSeconds"), 1800);
assert.equal(run("data.draft"), null);
assert.equal(intervals.size, 0);
now += 60000;
assert.equal(run("data.sessions[0].durationSeconds"), 1800);
const before = run("JSON.stringify(data.sessions)");
run(
  "data.draft={name:'Legacy workout',date:today(),exercises:[{name:'Squat',sets:[{weight:80,reps:5,done:true}]}]};validate(data)",
);
assert.match(run("workoutClock(data.draft)"), /Start timer now/);
run("finish()");
assert.equal(run("data.sessions[1].durationSeconds"), undefined);
...
run("syncTimer()");
elements.get("#start-timer").onclick();
assert.equal(run("data.draft.startedAt"), now);
run("data.draft=null;syncTimer()");
assert.equal(intervals.size, 0);
console.log(
  "Timer checks passed: formatting, background time, reload, single interval, finish, save failure, legacy data, validation, discard.",
);
